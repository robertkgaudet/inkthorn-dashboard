#!/usr/bin/env node
/**
 * generate-baton-rouge-demo-sites.js
 *
 * Takes leads from data/baton-rouge-leads.json and generates a professional
 * demo website for each business at crustacean/demos-br/[slug]/index.html
 *
 * Usage:
 *   node scripts/generate-baton-rouge-demo-sites.js             # generates all new leads
 *   node scripts/generate-baton-rouge-demo-sites.js --max 10    # first 10 only
 *   node scripts/generate-baton-rouge-demo-sites.js --id <id>   # single business
 */

const fs   = require('fs');
const path = require('path');

const LEADS_FILE  = path.join(__dirname, '..', 'data', 'baton-rouge-leads.json');
const DEMOS_DIR   = path.join(__dirname, '..', 'crustacean', 'demos-br');
const BASE_URL    = 'https://inkthorn.ai/crustacean/demos-br'; // GitHub Pages URL
...
const INDUSTRY_COPY = {
  ...
  roofing_contractor: { hero: 'Storm Season Is Marketing Season.', sub: 'Be the roofer Baton Rouge finds first when the next storm hits.' },
...
function main() {
  if (!fs.existsSync(LEADS_FILE)) {
    console.error('❌ No leads file found. Run scrape-baton-rouge-leads.js first.');
    process.exit(1);
  }
  ...
  console.log(`\n🎯 InkThorn Crustacean — Baton Rouge Demo Site Generator`);
  ...
  for (const lead of newLeads) {
    ...
    console.log(`  ✅ ${lead.name.padEnd(45)} → crustacean/demos-br/${slug}/`);
  }
  ...
  console.log(`📁 Location: crustacean/demos-br/`);
  console.log(`🌐 Live at: https://inkthorn.ai/crustacean/demos-br/[business-name]/\n`);
}
