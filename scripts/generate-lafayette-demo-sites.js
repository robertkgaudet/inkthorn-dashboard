#!/usr/bin/env node
/**
 * generate-lafayette-demo-sites.js
 *
 * Takes leads from data/lafayette-leads.json and generates a professional
 * demo website for each business at crustacean/demos-lafayette/[slug]/index.html
 *
 * Usage:
 *   node scripts/generate-lafayette-demo-sites.js             # generates all new leads
 *   node scripts/generate-lafayette-demo-sites.js --max 10    # first 10 only
 *   node scripts/generate-lafayette-demo-sites.js --id <id>   # single business
 */

const fs   = require('fs');
const path = require('path');

const LEADS_FILE  = path.join(__dirname, '..', 'data', 'lafayette-leads.json');
const DEMOS_DIR   = path.join(__dirname, '..', 'crustacean', 'demos-lafayette');
const BASE_URL    = 'https://inkthorn.ai/crustacean/demos-lafayette'; // GitHub Pages URL
...
const INDUSTRY_COPY = {
  ...
  roofing_contractor: { hero: 'Storm Season Is Marketing Season.', sub: 'Be the roofer Lafayette finds first when the next storm hits.' },
...
<head>
  <meta charset="utf-8" />
  <title>${lead.name} — ${city} ${categoryDisplay}</title>
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <meta name="description" content="${lead.name} — ${categoryDisplay} in ${city}. ${lead.address}. Call us: ${lead.phone || 'See details below'}." />
...
<body>
  ...
  <section class="hero">
    <div class="container">
      <div class="hero-cat">${categoryDisplay} · ${city}</div>
  ...
  <div class="info-bar">
    <div class="container info-bar-inner">
      ...
      <div class="info-item"><span class="info-icon">🌐</span> ${city}, Louisiana</div>
    </div>
  </div>
      ...
      <div class="two-col">
        <div>
          ...
          <p class="section-body">
            ${escHtmlNode(lead.name)} is a trusted ${categoryDisplay.toLowerCase()} serving the ${city} community.
        ...
        <div style="background:var(--gray-50);border-radius:16px;padding:36px;text-align:center;">
          ...
          <p style="color:var(--gray-500);font-size:0.9rem;">Trusted by ${city} customers.<br>Find us on Google Maps.</p>
...
function main() {
  if (!fs.existsSync(LEADS_FILE)) {
    console.error('❌ No leads file found. Run scrape-lafayette-leads.js first.');
    process.exit(1);
  }
  ...
  console.log(`\n⚜️  InkThorn Crustacean — Lafayette Demo Site Generator`);
  ...
  for (const lead of newLeads) {
    ...
    console.log(`  ✅ ${lead.name.padEnd(45)} → crustacean/demos-lafayette/${slug}/`);
  }
  ...
  console.log(`📁 Location: crustacean/demos-lafayette/`);
  console.log(`🌐 Live at: https://inkthorn.ai/crustacean/demos-lafayette/[business-name]/\n`);
}
